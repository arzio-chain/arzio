...
var (
	...
	MainnetFlag = cli.BoolFlag{
		Name:  "mainnet",
		Usage: "Arzio Mainnet network",
	}
	TestnetFlag = cli.BoolFlag{
		Name:  "testnet",
		Usage: "Arzio Testnet network",
	}
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	if !ctx.GlobalIsSet(RaftModeFlag.Name) && len(cfg.StaticNodes) == 0 {
		if ctx.GlobalIsSet(MainnetFlag.Name) {
			setArzioMainnetDefaultPeers(ctx, cfg)
		} else if ctx.GlobalIsSet(TestnetFlag.Name) {
			setArzioTestnetDefaultPeers(ctx, cfg)
		}
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	case ctx.GlobalBool(MainnetFlag.Name):
		...
		cfg.Genesis = core.DefaultArzioMainnetGenesisBlock()
	...
	case ctx.GlobalBool(TestnetFlag.Name):
		...
		cfg.Genesis = core.DefaultArzioTestnetGenesisBlock()
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	switch {
	case ctx.GlobalBool(MainnetFlag.Name):
		genesis = core.DefaultArzioMainnetGenesisBlock()
	case ctx.GlobalBool(TestnetFlag.Name):
		genesis = core.DefaultArzioTestnetGenesisBlock()
		// case ctx.GlobalBool(DeveloperFlag.Name):
...
func setArzioMainnetDefaultPeers(ctx *cli.Context, cfg *p2p.Config) {
...
func setArzioTestnetDefaultPeers(ctx *cli.Context, cfg *p2p.Config) {
	cfg.StaticNodes = append(cfg.StaticNodes,
